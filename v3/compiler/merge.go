...
func MergeData(inputList, outputList *model.DataTableList, symbols *model.TypeTable) {
	...
	for _, inputTab := range inputList.AllTables() {
		...
		// 遍历输入表的每一行
		outputTab.KValueCells = make([][]*model.Cell, len(inputTab.Rows))
		for row := 1; row < len(inputTab.Rows); row++ {
			...
			for _, inputHeader := range inputTab.Headers {
				...
				// 记录所有键值表值所在的col
				if inputHeader.TypeInfo.ObjectType == "KVDefine" && inputHeader.TypeInfo.FieldName == "Value" {
					if outputTab.KValueCells[row] == nil {
						outputTab.KValueCells[row] = []*model.Cell{}
					}
					outputTab.KValueCells[row] = append(outputTab.KValueCells[row], inputCell)
				}
...
func combineArrayCell(outputCell, inputCell *model.Cell, splitter string) {
	...
	// 把之前的格子的值合并为字符串
	for c := outputCell.Next; c != nil; c = c.Next {
